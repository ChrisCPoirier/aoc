...
func (g *gridLayout) Layout(objects []fyne.CanvasObject, size fyne.Size) {
	...
	padWidth := float32(0.0)
	padHeight := float32(0.0)
...
func (g *gridLayout) MinSize(objects []fyne.CanvasObject) fyne.Size {
	...
	if g.horizontal() {
		...
		return minContentSize.Add(fyne.NewSize(fyne.Max(float32(g.Cols-1), 0), fyne.Max(float32(rows-1), 0)))
	}

	minContentSize := fyne.NewSize(minSize.Width*float32(rows), minSize.Height*float32(g.Cols))
	return minContentSize.Add(fyne.NewSize(fyne.Max(float32(rows-1), 0), fyne.Max(float32(g.Cols-1), 0)))
}
